    ...
    :bStart_backend(true)
{
    ...
    string sConfig_file = sConfig_file_ + "config.yaml";
...
void System::PubSimuImageData(double dStampSec, cv::Mat &img, vector<cv::Point2f> Points)
{
    if (!init_feature)
    {
        cout << "1 PubImageData skip the first detected feature, which doesn't contain optical flow speed" << endl;
    ...
    double last_time = last_image_time;
    last_image_time = dStampSec;
    // frequency control
    if (round(1.0 * pub_count / (dStampSec - first_image_time)) <= FREQ)
    {
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        //int id=0;
        static std::map<int,cv::Point2f> cur_un_pts;
        static std::map<int,cv::Point2f> pre_un_pts;
        for(size_t i=0;i< Points.size();i++)
        {
           feature_points->points.push_back(Eigen::Vector3d(Points[i].x,Points[i].y,1));
           feature_points->id_of_point.push_back(i);
           feature_points->u_of_point.push_back(Points[i].x);
           feature_points->v_of_point.push_back(Points[i].y);

           float v_x=0;
           float v_y=0;

           float dt = dStampSec - last_time;
           cur_un_pts.insert(std::make_pair(i,Points[i]));
           if(!pre_un_pts.empty())
           {
               const auto &iter = pre_un_pts.find(i);
               if(iter!=pre_un_pts.end())
               {
                   v_x = (Points[i].x - iter->second.x)/dt;
                   v_y = (Points[i].y - iter->second.y)/dt;
               }
           }
           feature_points->velocity_x_of_point.push_back(v_x);
           feature_points->velocity_y_of_point.push_back(v_y);
        }
        pre_un_pts = cur_un_pts;
        if (!init_pub)
        {
            cout << "4 PubImage init_pub skip the first image!" << endl;
            init_pub = 1;
    ...
    // // cout << "3 PubImageData t : " << dStampSec << endl;
    // trackerData[0].readSimuImageFeature(img,points,dStampSec);
	...
	if (SHOW_TRACK)
	{
        ...
        // {
		// 	double len = min(1.0, 1.0 * 10 / WINDOW_SIZE);
        //     //cout<< trackerData[0].cur_pts[j].x<<" "<<trackerData[0].cur_pts[j].y<<endl;
        //     if(Points[j].x< 640 && Points[j].x > 0 && Points[j].y > 0 && Points[j].y < 640 )
		// 	    // cv::circle(show_img,Points[j], 2, cv::Scalar(255 * (1 - len), 0, 255 * len), 2);
        //         cv::circle(show_img, Points[j], 2, cv::Scalar(255 , 0, 255), 2);
