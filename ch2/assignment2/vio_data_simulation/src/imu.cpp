...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    std::ofstream save_points1,save_points2;
    save_points2.open(dist+"_euler.txt");
    save_points1.open(dist+"_mid.txt");
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        save_points1<<imupose.timestamp<<" "
                   <<Qwb.w()<<" "
                   <<Qwb.x()<<" "
                   <<Qwb.y()<<" "
                   <<Qwb.z()<<" "
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        // /// 中值积分
        // MotionData imuposepuls=imudata[i+1];
        // Eigen::Vector3d imu_gyromid=(imupose.imu_gyro+imuposepuls.imu_gyro)/2;       //w=1/2(wk-wk_biase+wk+1-wk+1_bias)
        // Eigen::Quaterniond dq;
        // Eigen::Vector3d dtheta_half=imu_gyromid*dt/2;
        // dq.w() = 1;
        // dq.x() = dtheta_half.x();
        // dq.y() = dtheta_half.y();
        // dq.z() = dtheta_half.z();
        // Eigen::Quaterniond Qwbpause=Qwb*dq;             // qk+1=qk*deltaq
        // Eigen::Vector3d acc_w=((Qwb*imupose.imu_acc+gw)+(Qwbpause*imuposepuls.imu_acc+gw))/2;    //aw = ((Rkwb * ( acck_body - acck_bias ) + gw)+(Rk+1wb * ( acck+!_body - acck+1_bias ) + gw))/2
        // Qwb=Qwbpause;
        // Vw=Vw+acc_w*dt;
        // Pwb=Pwb+Vw*dt+0.5*dt*dt*acc_w;
        ...
        //　按着imu postion, imu quaternion , cam postion, cam quaternion 的格式存储，由于没有cam，所以imu存了两次
        save_points2<<imupose.timestamp<<" "
                   <<Qwb.w()<<" "
                   <<Qwb.x()<<" "
                   <<Qwb.y()<<" "
                   <<Qwb.z()<<" "
